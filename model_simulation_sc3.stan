...
model {
    ...
    for (j in 1:p){
        ...
        target += multi_normal_lpdf(gamma[j] | rep_vector(0, psi), diag_matrix(rep_vector(1, psi)) * tau[j] * sigma); // target += normal_lpdf(sum(gamma[j]) |  0, 0.01*psi)
        if (j < 2 && j > 3) {target += normal_lpdf(sum(gamma[j]) | 0, 0.01*psi);}
    }
}
