...
C <- matrix(c(1,  0.3,  0.0,  0.0, -0.04,
           0.61,    1, 0.16, 0.15,  0.06,
            0.0, 0.16,    1, 0.99,  0.48,
...
model {
  ...
  for (i in 1:n){
   ...
   target += -1*log(1-student_t_cdf(u[i], alpha[i], 0, 1));
  }
  // target += pareto(y | u, alpha);
...
generated quantities {
  ...
  array[grid_n] real <lower=0> gridalpha; // new tail index
...
for(iter in 1:total.iter){
  ...
  quant.evgam <- evgam::evgam(y ~ s(X1, bs="ts", k=10) +
                                  s(X2, bs="ts", k=10) +
                                  s(X3, bs="ts", k=10) +
                                  s(X4, bs="ts", k=10) +
                                  s(X5, bs="ts", k=10), 
                    data = A.df, family = "ald", ald.args = list(tau = threshold))
  ...
  newx <- seq(0,1,length.out = newx.length)
  ...
  g1 <- g1.l + g1.nl
  g5 <- g5.l + g5.nl
  eta.g <- rep(theta.adjusted[1], newx.length) + g1 + g2.l + g5
  alp.new <- as.vector(exp(theta.origin[1] + xholder %*% theta.origin[-1] + f1(newx) + f5(newx)))
  # alp.new <- exp(eta.g)
  grid.zero <- rep(0, newx.length)
...
# save(newgsmooth.container, alpha.container, mise.container, evgam.1.container, evgam.scale.container, mise.evgam.1.container, mise.evgam.scale.container, vgam.1.container, vgam.scale.container, mise.vgam.1.container, mise.vgam.scale.container, file=paste0("evgam_mc_scC2_",n.origin,"_",array.id ,".Rdata"))
load(paste0("./simulation/results/2026-02-10_evgam_mc_scC2_",(n.origin*0.05),".Rdata"))
...
# ggsave(paste0("./simulation/results/",Sys.Date(),"_",total.iter,"_MC_evgam_scC2_",n.origin, ".pdf"), width=9.5, height = 7.78)
