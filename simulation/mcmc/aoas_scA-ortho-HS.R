...
sum(t(bs.linear[,c(1:2)]) %*% bs.nonlinear[,c(1:psi)])
...
init.alpha <- list(list(gamma= array(rep(0.2, (psi*p)), dim=c(p,psi)),
                        theta = rep(-0.1, (p+1)), sigma_lasso = 1, 
                   ...
                   list(gamma = array(rep(-0.15, (psi*p)), dim=c(p,psi)),
                        theta = rep(0.05, (p+1)), sigma_lasso = 0.5,
                        lambda1 = rep(2,p), lambda2 = rep(5,p),
                        t1 = 1, t2 = 1, b1 = 0.1, b2 = 0.01,
                        lt1 = rep(0.1, p), lt2 = rep(0.1, p)),
                   list(gamma = array(rep(-0.75, (psi*p)), dim=c(p,psi)),
                        theta = rep(0.01, (p+1)), sigma_lasso = 1,
                        lambda1 = rep(0.5,p), lambda2= rep(3,p),
...
ggplot(data.scenario, aes(x=x)) + 
  ...
  geom_line(aes(y=q2, col = "Posterior Mean"), linewidth=1.5) +
  scale_color_manual(values=c("steelblue", "red")) + 
  scale_fill_manual(values=c("steelblue"), name = "") +
