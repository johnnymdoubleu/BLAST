...
for(iter in 1:total.iter){
  ...
  g2 <- g2.l + g2.nl
  g3 <- g3.l + g3.nl
  eta.g <- rep(theta.adjusted[1], n) + g2 + g3 + g4.l + g5.l
  alp.new <- exp(eta.g)
  grid.zero <- rep(0, n)
  g.new <- c(grid.zero, g2, g3, g4.l, g5.l)
  l.new <- c(grid.zero, g2.l, g3.l, g4.l, g5.l)
  ...
  init.alpha <- list(list(gamma_raw= array(rep(0.2, (psi*p)), dim=c(p,psi)),
                    ...
                    list(gamma_raw = array(rep(0.15, (psi*p)), dim=c(p,psi)),
                          theta = rep(-0.05, (p+1)), tau = rep(0.3, p), #rho = 1,
...
# load(paste0("./simulation/results/MC-Scenario_A/2026-01-24_",total.iter,"_MC_scD_",n.origin,".Rdata"))
...
# # ggsave(paste0("./simulation/results/",Sys.Date(),"_",total.iter,"_MC_linear_scD_",n.origin,".pdf"), width=12.5, height = 7.78)                
...
# save(alpha.container, gridgnl.container, gridgl.container, gridgsmooth.container, mise.container, qqplot.container, file = paste0(Sys.Date(),"_",total.iter,"_MC_scD_",n.origin,".Rdata"))
