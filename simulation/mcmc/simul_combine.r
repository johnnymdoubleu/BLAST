...
true.container <- matrix(, nrow=grid.n, ncol=0)
...
for (f in file_list) {
  ...
  if(EV == TRUE){
    tryCatch({
      ...
      true.container <- cbind(true.container, temp_env$true.container[,c(1:iter)])
...
if(EV==FALSE){  
  ...
  total.iter <- length(file_list) * iter
  colnames(alpha.container) <- paste0("V", 1:total.iter)
  colnames(true.container) <- paste0("V", 1:total.iter)
  ...
  # save(alpha.container, true.container, vgam.1.container, vgam.scale.container, evgam.1.container, evgam.scale.container, mise.container, mise.evgam.1.container, mise.evgam.scale.container, mise.vgam.1.container, mise.vgam.scale.container, file = paste0(Sys.Date(),"_evgam_mc_scD_",n,"_ct.Rdata"))  
}
