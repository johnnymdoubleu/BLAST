...
if(EV==TRUE){
  ...
  file_pattern <- paste0("2026-04-02_",iter,"_MC_scD_",n,"_.*.Rdata")
}
...
if(EV==FALSE){  
  ...
  # save(alpha.container, gridgnl.container, gridgl.container, gridgsmooth.container, mise.container, file = paste0(Sys.Date(),"_",total.iter,"_MC_scD_",n,"-cts.Rdata"))
} else {
  total.iter <- length(file_list) * iter
  colnames(alpha.container) <- paste0("V", 1:total.iter)
