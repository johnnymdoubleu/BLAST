...
model.stan <- "// Stan model for BLAST Student-T Uncorrelated Samples
data {
...
vgam.1.container <- evgam.1.container <- as.data.frame(matrix(, nrow = (n*(1-threshold)), ncol = total.iter))
vgam.scale.container <- evgam.scale.container <- as.data.frame(matrix(, nrow = (n*(1-threshold)), ncol = total.iter))
...
for(iter in 1:total.iter){
  ...
  mise.evgam.scale.container[iter] <- auc(newx, ((1/alp.new)-evgam.xi.scale)^2, type = "spline")
}
