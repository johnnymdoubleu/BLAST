...
knots <- matrix(NA, nrow = psi, ncol =p)
...
for(i in 1:p){
    ...
    splines <- basis.tps(newx, test.knot, m=2, rk=TRUE, intercept = TRUE)
    xholder.linear <- cbind(xholder.linear, splines[,1:no.theta])
    xholder.nonlinear <- cbind(xholder.nonlinear, splines[,-c(1:no.theta)])
    knots <- seq(min(x.origin[,i]), max(x.origin[,i]), length.out = psi)  
    tps <- basis.tps(x.origin[,i], knots, m = 2, rk = TRUE, intercept = TRUE)
    bs.linear <- cbind(bs.linear, tps[,1:no.theta])
    bs.nonlinear <- cbind(bs.nonlinear, tps[,-c(1:no.theta)])  
}
...
for(j in 1:p){
    f.linear.origin[,j] <- as.matrix(bs.linear[,(((j-1)*no.theta)+1):(((j-1)*no.theta)+no.theta)]) %*% theta.origin[,j]
    f.nonlinear.origin[,j] <- bs.nonlinear[, (((j-1)*psi)+1):(((j-1)*psi)+psi)] %*% gamma.origin[, j]
    f.origin[1:n, j] <- f.linear.origin[,j] + f.nonlinear.origin[,j]
...
log.posterior <- function(beta, y.origin){
  ...
  for(i in 1:n){
      first.lik[i] <- sum(g[i,]) - log(y.origin[i])
      second.lik[i] <- exp(sum(g[i,])) * log(y.origin[i]/u)
...
ggplot(df.theta, aes(x = seq)) + ylab("") + xlab("") +
  ...
  scale_x_discrete(breaks=c(seq(0, (no.theta*p), no.theta)+5), 
                    label = c(expression(bold(theta[1])),
...
print(plt.samp + 
      geom_line(aes(y = trueAlp, col = paste0("True Alpha:",n,"/",psi,"/",threshold)), linewidth = 2.5) + labs(col = "") +
