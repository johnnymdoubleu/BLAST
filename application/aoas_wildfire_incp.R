...
parameters {
    ...
    vector[p] intcpt; // linear predictor
...
generated quantities {
    ...
    for (j in 1:p){
        newgsmooth[,j] = xholderNonlinear[,(((j-1)*psi)+1):(((j-1)*psi)+psi)] * gamma[j] + xholderLinear[,j] * theta[j] + intcpt[j];
    };    

    for (i in 1:n){ 
        newalpha[i] = exp(sum(newgsmooth[i,]));
    };
}
"
...
init.alpha <- list(list(gammaTemp = array(rep(0, ((psi-2)*p)), dim=c(p, (psi-2))),
                   ...
                   list(gammaTemp = array(rep(0, ((psi-2)*p)), dim=c(p, (psi-2))),
                        theta = rep(0.1, p), intcpt = rep(0.1, p),
...
intercept.samples <- summary(fit1, par=c("intcpt"), probs = c(0.05,0.5, 0.95))$summary
