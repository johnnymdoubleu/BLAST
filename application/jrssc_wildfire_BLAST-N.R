...
transformed parameters {
  ...
  array[p] vector[psi] gamma;
  {
    ...
    for (j in 1:p){
      for (k in 1:psi){
        int idx = (j-1)*psi + k;
        gamma[j][k] = gamma_raw[j][k] * Z_scales[idx];
      }; 
...
data.stan <- list(y = as.vector(y), u = u, p = p, n= n, psi = psi, Z_scales= Z_scales,
                  ...
                  X_means = X_means, X_sd = X_sd, X_min=fwi.min,
...
elpd.loo
save(elpd.loo, file = (paste0("./BLAST/application/BRIT_full_",Sys.Date(),"_",psi+2,"_",floor(threshold*100),"quantile_IC.Rdata")))
