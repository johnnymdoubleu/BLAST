...
transformed parameters {
    ...
    for (i in 1:n){
        alpha[i] = exp(theta[1] + sum(gsmooth[i,]) + epsilon[i]); 
    };
}
...
model {
    ...
    target += lognormal_lpdf(prec | log(100), 2); // target += gamma_lpdf(prec | 1, 1e-5)
...
generated quantities {
    ...
    for (i in 1:n){
      ...
      newalpha[i] = exp(theta[1] + sum(newgsmooth[i,]) + newepsilon[i]);
    }
