...
# --- 1. Data Preparation & Parameter Extraction ---
draws_all <- as.matrix(fit1)
n_draws   <- nrow(draws_all)

alpha0   <- draws_all[, "theta[1]"]
beta_lin <- draws_all[, paste0("theta[", 2:(p+1), "]")]

gamma_names <- grep("^gamma\\[", colnames(draws_all), value = TRUE)
gamma_names_sorted <- gamma_names[order(
  as.numeric(gsub("gamma\\[(\\d+),.*", "\\1", gamma_names)),
  as.numeric(gsub("gamma\\[.*,(\\d+)\\]", "\\1", gamma_names))
)]
beta_nonlin <- draws_all[, gamma_names_sorted]

post_beta_full <- cbind(beta_lin, beta_nonlin) 

# --- 2. Construct Projection Matrices ---
X_projection <- cbind(bs.linear[, -1], bs.nonlinear)
colnames(X_projection) <- c(paste0("beta[", 1:p, "]"), gamma_names_sorted)

group_list <- lapply(1:p, function(j) {
  lin_idx <- j
  nonlin_indices <- (p + (j-1)*psi + 1):(p + j*psi)
  return(c(lin_idx, nonlin_indices))
})

# --- 3. Group-Wise Importance (Signal-to-Noise Ranking) ---
importance_scores <- sapply(group_list, function(indices) {
  group_draws <- post_beta_full[, indices]
  mag <- sqrt(rowSums(group_draws^2)) # L2 Norm
  return(mean(mag) / sd(mag))
})
names(importance_scores) <- covariates
cov_ranking   <- order(importance_scores, decreasing = TRUE)
selected_vars <- cov_ranking[1:p] # Top 7
selected_col_indices <- unlist(group_list[selected_vars])

X_sub <- X_projection[, selected_col_indices]
X_sub_with_int <- cbind(1, X_sub)
X_full_with_int <- cbind(1, X_projection)
n_proj_cols    <- ncol(X_sub_with_int)
beta_full_with_int <- cbind(alpha0, post_beta_full)
log_alpha_full <- X_full_with_int %*% t(beta_full_with_int) 

# --- 4. Custom KL-Divergence Projection (Numerical Optimization) ---
# Objective: Maximize expected Pareto log-likelihood
pareto_kl_objective <- function(params, X_matrix, y_obs, u_thresh) {
  # Perform matrix multiplication
  log_alpha_sub <- X_matrix %*% params
  
  # CRITICAL: Convert the resulting N x 1 matrix into a vector of length N
  alpha_sub <- as.vector(exp(log_alpha_sub))
  
  # Ensure observations and thresholds are also flat vectors
  y_obs    <- as.vector(y_obs)
  u_thresh <- as.vector(u_thresh)
  
  # Now all components are vectors of length 366
  term1 <- log(alpha_sub)
  term2 <- log(u_thresh)
  term3 <- (alpha_sub + 1) * log(y_obs / u_thresh)
  
  neg_log_lik <- -sum(term1 - term2 - term3)
  
  # Numerical stability: handle potential overflows/NaNs
...
X_sub_with_int <- cbind(1, X_sub)
message("Projecting via Latent Matrix Algebra...")

# 3. The "Hat Matrix" approach:
# We solve (X'X)^-1 X'Y where Y is the full model's linear predictor.
# This minimizes the squared error on the log-scale, which we 
# rationalized as the KL-divergence minimizer for this Gaussian-latent posterior.
xtx_inv <- solve(t(X_sub_with_int) %*% X_sub_with_int)
projected_beta_sub <- t(xtx_inv %*% t(X_sub_with_int) %*% log_alpha_full)
...
for (i in 1:p) {
  ...
  b_lin_proj    <- projected_beta_sub[, current_col]
  b_nonlin_proj <- projected_beta_sub[, (current_col + 1):(current_col + psi)]
  
  lin_effect    <- xholder.linear[, var_idx + 1] %*% t(b_lin_proj)
  nonlin_effect <- xholder.nonlinear[, ((var_idx-1)*psi + 1):(var_idx*psi)] %*% t(b_nonlin_proj)
  total_effect  <- lin_effect + nonlin_effect
  
  plot_data[[i]] <- data.frame(
    x = newx,
    median = apply(total_effect, 1, median),
    lower  = apply(total_effect, 1, quantile, 0.025),
    upper  = apply(total_effect, 1, quantile, 0.975),
    covariate = var_name
  )
  current_col <- current_col + 1 + psi
}

# --- 6. Plotting Tail Index Alpha over Grid ---
lp_grid <- matrix(projected_beta_sub[, 1], nrow = length(newx), ncol = n_draws, byrow = TRUE)
current_col <- 2
for (i in 1:p) {
  var_idx <- selected_vars[i]
  b_lin_proj    <- projected_beta_sub[, current_col]
  ...
  lp_grid <- lp_grid + (xholder.linear[, var_idx + 1] %*% t(b_lin_proj)) +
             (xholder.nonlinear[, ((var_idx-1)*psi + 1):(var_idx*psi)] %*% t(b_nonlin_proj))
  current_col <- current_col + 1 + psi
}

alpha_proj_grid <- exp(lp_grid)
plot_alpha_df <- data.frame(
  x = newx,
  median = apply(alpha_proj_grid, 1, median),
  low = apply(alpha_proj_grid, 1, quantile, 0.025),
  high = apply(alpha_proj_grid, 1, quantile, 0.975)
)

# --- 7. Final Plots ---
# Plot 1: Individual Smooths
ggplot(bind_rows(plot_data), aes(x = x, y = median)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "steelblue", alpha = 0.2) +
  geom_hline(aes(yintercept=0), color = "grey2", linetype=2, linewidth=0.8) +
  geom_line(color = "steelblue", linewidth = 1.1) +
  facet_wrap(~covariate, scales = "free", nrow = 2) + ylab(expression(g[j](c))) + xlab(expression(c)) +
  theme_minimal(base_size = 30)

# Plot 2: Total Alpha(x)
# ggplot(plot_alpha_df, aes(x = x, y = median)) +
#   geom_ribbon(aes(ymin = low, ymax = high), fill = "steelblue", alpha = 0.2) +
#   geom_line(color = "steelblue", linewidth = 1.2) +
#   theme_minimal() + labs(title = expression("Projected Tail Index " * alpha * (x)), x = "Grid", y = expression(alpha))

importance_df <- data.frame(
  covariate = names(importance_scores),
  score = as.numeric(importance_scores)
) %>%
  arrange(desc(score)) # Sort by importance

# 2. Factorize covariate names to maintain the sorted order in the plot
importance_df$covariate <- factor(importance_df$covariate, levels = importance_df$covariate)

# 3. Highlight the top K (selected) variables
k_cov <- p
importance_df$selected <- ifelse(1:nrow(importance_df) <= k_cov, "Selected", "Excluded")

# 4. Generate the Plot
ggplot(importance_df, aes(x = covariate, y = score, fill = selected)) +
  geom_bar(stat = "identity", alpha = 0.8) +
  scale_fill_manual(values = c("Selected" = "steelblue", "Excluded" = "grey70")) +
  coord_flip() + # Flip for better readability of covariate names
  theme_minimal(base_size = 14) +
  labs(
    title = "Covariate Importance Ranking",
    subtitle = "Signal-to-Noise Ratio (Mean Magnitude / SD)",
    x = "Covariate",
    y = "Importance Score",
    fill = "Status"
  ) +
