...
parameters {
    ...
    array[p] real <lower=0> lambda1; // horseshoe penalty
    array[p] real <lower=0> lambda2; // group horseshoe penalty
...
transformed parameters {
    ...
    matrix[n, p] gsmooth; // linear component
    {
      ...
      for(j in 1:p){
          ...
          gammaFL[j] = basisFL[, (((j-1)*2)+1):(((j-1)*2)+2)] * (bsNonlinear[indexFL[(((j-1)*2)+1):(((j-1)*2)+2)], (((j-1)*psi)+2):(((j-1)*psi)+(psi-1))] * gammaTemp[j]) * (-1);
