...
```{r}
# xholder$BA <- seq(0,1, length.out = n)
# fwi.df <- 
plot(fwi.df$FWI, fwi.df$BA)

# Predict each quantile curve and plot
colss <- rainbow(length(taus))
for(i in 1:length(taus)){
  pred <- qdo(m.gam, taus[i], predict, newdata=xholder)
  lines(fwi.df$FWI, pred, col = colss[i])
}
```

```{r}
qdo(m.gam, 0.975, cqcheck)
```
