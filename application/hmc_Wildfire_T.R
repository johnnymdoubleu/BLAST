...
file <- file.path(cmdstan_path(), "model_t.stan")
...
fit1 <- stan(
    file = "model_t.stan",  # Stan program
...
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_theta.pdf"), width=10, height = 7.78)
...
ggplot(df.gamma, aes(x =labels, y = m, color = covariate)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 20),
          ...
          axis.text = element_text(size = 28))
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_gamma.pdf"), width=10, height = 7.78)
...
ggplot(data.smooth, aes(x=x, group=interaction(covariates, replicate))) + 
  ...
  theme(legend.position = "none",
          ...
          axis.text = element_text(size = 20))
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_smooth.pdf"), width=12.5, height = 15)
...
ggplot(data.nonlinear, aes(x=x, group=interaction(covariates, replicate))) + 
  ...
  theme(legend.position = "none",
          ...
          strip.text = element_blank(),
          axis.text = element_text(size = 20))
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_nonlinear.pdf"), width=12.5, height = 15)
...
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_alpha.pdf"), width=10, height = 7.78)
...
ggplot(data = data.frame(grid = grid, l.band = l.band, trajhat = trajhat, 
  ...
  coord_fixed(xlim = c(-3, 3),  
              ylim = c(-3, 3))
# ggsave(paste0("./BRSTIR/application/figures/",Sys.Date(),"_student-t_mcmc_qqplot.pdf"), width=10, height = 7.78)
