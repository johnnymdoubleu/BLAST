...
beta.emp <- c(rep(0, (p+1)), rep(0, p*psi), 0.05, 30)
...
# ggsave(paste0("./BRSTIR/application/figures/",date,"_map_alpha.pdf"), width=10, height = 7.78)
...
ocode<-"
  ...
  transformed parameters {
      ...
      for (i in 1:n){
          alp[i] <- exp(theta[1] + dot_product(bsLinear[i], theta[2:newp]) + (gsmooth[i,] *rep_vector(1, p)));
      };
  }
  ...
  model {
      // likelihood
      for (i in 1:n){
          target += pareto_lpdf(y[i] | u, alp[i]);
      }
